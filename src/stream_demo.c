...
void *detect_in_thread(void *ptr)
{
    ...
    free_image(det_s);
    convert_stream_detections(avg, l.classes, l.n, l.sqrt, l.side, 1, 1, demo_thresh, probs, boxes, 0);
...
void demo_stream(char *cfgfile, char *weightfile, float thresh, int cam_index, const char *filename, char **names, image *labels, int classes, int frame_skip)
{
    //skip = frame_skip;
    ...
    while(1){
        ...
        if(1){
            ...
            if(socketFlag){
            	int rst = send_image_socket(disp);
            	if(rst <= 0){
            		free_image(disp);
            		break;
        	    }
